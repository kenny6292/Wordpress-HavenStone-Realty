...
function havenstone_send_notification(string $subject, string $html, string $reply_to=''): bool {
    ...
    // Use Resend when production credentials are configured; retain wp_mail as a safe fallback.
    $api_key = defined('HAVENSTONE_RESEND_API_KEY') ? HAVENSTONE_RESEND_API_KEY : getenv('RESEND_API_KEY');
    $resend_from = defined('HAVENSTONE_RESEND_FROM') ? HAVENSTONE_RESEND_FROM : getenv('HAVENSTONE_RESEND_FROM');
    if ($api_key && $resend_from && function_exists('wp_remote_post')) {
        $payload = ['from' => $resend_from, 'to' => [$to], 'subject' => $subject, 'html' => $html];
        if ($reply_to && is_email($reply_to)) $payload['reply_to'] = [$reply_to];
        $response = wp_remote_post('https://api.resend.com/emails', [
            'timeout' => 15,
            'headers' => ['Authorization' => 'Bearer ' . $api_key, 'Content-Type' => 'application/json'],
            'body' => wp_json_encode($payload),
        ]);
        if (!is_wp_error($response) && wp_remote_retrieve_response_code($response) >= 200 && wp_remote_retrieve_response_code($response) < 300) return true;
    }
...
function havenstone_redirect_after_form(string $status): void {
    $target = wp_get_referer();
    if (!$target) $target = home_url('/');
    $target = remove_query_arg(['havenstone_status','_wpnonce'], $target);
    wp_safe_redirect(add_query_arg('havenstone_status', $status, $target));
    exit;
}

function havenstone_form_rate_limited(string $key, string $email): bool {
    $bucket = 'havenstone_' . md5($key . '|' . strtolower($email) . '|' . ($_SERVER['REMOTE_ADDR'] ?? ''));
    if (get_transient($bucket)) return true;
...
function havenstone_handle_enquiry(): void {
    ...
    if (havenstone_form_rate_limited('enquiry', $email)) return;
    ...
    if($id){
        ...
        havenstone_redirect_after_form('enquiry_sent');
    }
}
...
function havenstone_handle_viewing_request(): void {
 ...
 if (!empty($_POST['website'])) return;
 if (havenstone_form_rate_limited('viewing', $email)) return;
 ...
 if($id){foreach(['email'=>$email,'phone'=>$phone,'date'=>$date,'time'=>$time,'property_id'=>$property_id] as $k=>$v)update_post_meta($id,'_havenstone_'.$k,$v); update_post_meta($id,'_havenstone_lead_status','new');
   ...
   havenstone_redirect_after_form('viewing_sent');
 }
}
...
function havenstone_handle_property_request(): void {
    ...
    if (!empty($_POST['website'])) return;
    if (havenstone_form_rate_limited('property_request', $email)) return;
    ...
    if (!$target) $target = home_url('/');
    $target = remove_query_arg(['havenstone_status','_wpnonce'], $target);
    wp_safe_redirect(add_query_arg('havenstone_status', 'property_request_sent', $target));
    exit;
}
