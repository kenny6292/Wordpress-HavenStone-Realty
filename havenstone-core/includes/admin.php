...
function havenstone_register_admin_meta_boxes(): void {
    foreach (['enquiry' => 'Enquiry Details', 'viewing_request' => 'Viewing Request Details'] as $post_type => $title) {
        add_meta_box(
            'havenstone_' . $post_type . '_details',
            __($title, 'havenstone'),
            'havenstone_render_lead_details',
            $post_type,
            'normal',
            'high'
        );
    ...
    foreach (['enquiry', 'viewing_request'] as $post_type) {
        add_meta_box(
            'havenstone_' . $post_type . '_actions',
            __('Lead Actions', 'havenstone'),
            'havenstone_render_lead_actions',
            $post_type,
            'side',
            'high'
        );
...
function havenstone_render_lead_details(WP_Post $post): void {
    ...
    $request_type = get_post_meta($post->ID, '_havenstone_request_type', true);
    $property_type = get_post_meta($post->ID, '_havenstone_property_type', true);
    $location = get_post_meta($post->ID, '_havenstone_location', true);
    $min_price = get_post_meta($post->ID, '_havenstone_min_price', true);
    $max_price = get_post_meta($post->ID, '_havenstone_max_price', true);
    $bedrooms = get_post_meta($post->ID, '_havenstone_bedrooms', true);
    $bathrooms = get_post_meta($post->ID, '_havenstone_bathrooms', true);
    ...
    if ($date) {
        echo '<p><strong>Preferred Date:</strong> ' . esc_html($date) . '</p>';
    }
    if ($time) {
        echo '<p><strong>Preferred Time:</strong> ' . esc_html($time) . '</p>';
    }

    if ($request_type) {
        echo '<hr><p><strong>Request Type:</strong> ' . esc_html(ucwords(str_replace(['-', '_'], ' ', $request_type))) . '</p>';
    }
    if ($property_type) {
        echo '<p><strong>Property Type:</strong> ' . esc_html($property_type) . '</p>';
    }
    if ($location) {
        echo '<p><strong>Preferred Location:</strong> ' . esc_html($location) . '</p>';
    }
    if ($min_price !== '') {
        echo '<p><strong>Minimum Budget:</strong> ' . esc_html($min_price) . '</p>';
    }
    if ($max_price !== '') {
        echo '<p><strong>Maximum Budget:</strong> ' . esc_html($max_price) . '</p>';
    }
    if ($bedrooms !== '') {
    ...
    if ($property_id && get_post_type($property_id) === 'property') {
        echo '<p><strong>Property:</strong> <a href="' . esc_url(get_edit_post_link($property_id)) . '">' . esc_html(get_the_title($property_id)) . '</a> <a href="' . esc_url(get_permalink($property_id)) . '" target="_blank" rel="noopener">View</a></p>';
    } elseif ($property_id) {
...
function havenstone_render_lead_actions(WP_Post $post): void {
    ...
    echo '<p><label for="havenstone_lead_status"><strong>Status</strong></label></p>';
    echo '<p><select name="havenstone_lead_status" id="havenstone_lead_status" style="width:100%;">';
    foreach ([
        'new' => 'New',
        'contacted' => 'Contacted',
        'scheduled' => 'Scheduled',
    ...
    if ($email) {
        echo '<p><a class="button button-primary" href="mailto:' . esc_attr($email) . '">Email lead</a></p>';
    }
    if ($phone) {
        echo '<p><a class="button" href="tel:' . esc_attr($phone) . '">Call lead</a></p>';
    }
    if ($property_id && get_post_type($property_id) === 'property') {
        echo '<p><a class="button" href="' . esc_url(get_permalink($property_id)) . '" target="_blank" rel="noopener">Open property</a></p>';
    }
...
function havenstone_save_lead_details(int $post_id): void {
    if (!isset($_POST['havenstone_lead_details_nonce']) || !wp_verify_nonce(
        sanitize_text_field(wp_unslash($_POST['havenstone_lead_details_nonce'])),
        'havenstone_lead_details'
    )) {
        return;
    }
...
foreach (['enquiry', 'viewing_request'] as $type) {
    add_filter("manage_{$type}_posts_columns", 'havenstone_lead_columns');
}
...
foreach (['enquiry', 'viewing_request'] as $type) {
    add_action("manage_{$type}_posts_custom_column", 'havenstone_lead_column_content', 10, 2);
}
...
function havenstone_property_admin_column_content(string $column, int $post_id): void {
    if ($column === 'havenstone_price') {
        echo esc_html(get_post_meta($post_id, '_havenstone_price_label', true));
    }
    ...
    if ($column === 'havenstone_featured') {
        echo get_post_meta($post_id, '_havenstone_featured', true) === '1' ? 'Yes' : '—';
    }
}
...
function havenstone_admin_property_assets(string $hook): void {
    ...
    wp_enqueue_media();
    wp_enqueue_script(
        'havenstone-property-admin',
        plugins_url('../assets/js/admin-property.js', __FILE__),
        ['jquery'],
        HAVENSTONE_CORE_VERSION,
        true
    );
}
